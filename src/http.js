...
const http = (function () {
    ...
    PSocket.prototype.read = function () {
        ...
        if (this.readQueue.length) {
            ...
            const info = this.readQueue.shift();
            if (!info.resultCode) {
    ...
    const ary2utf8 = (function () {
        ...
        return function (ary) {
            ...
            while (cur < ary.length) {
                ...
                const rest = ary.slice(cur + 1, cur + pattern.bytes);
                cur += pattern.bytes;
                let code = '';
    ...
    const utf82ary = (function () {
        ...
        const code2utf8array = function (code) {
            let pattern = null;
            let code01 = code.toString(2);
            for (var i = 0, len = patterns.length; i < len; i++) {
    ...
    EventSource.prototype = {
        ...
        removeEventListener: function (type, callback) {
            ...
            for (let i = this.listeners_[type].length - 1; i >= 0; i--) {
    ...
    HttpServer.prototype = {
        ...
        readRequestFromSocket_: function (pSocket) {
            ...
            var onDataRead = function (data) {
                ...
                const headers = requestData.substring(0, endIndex).split('\n');
                const headerMap = {};
                // headers[0] should be the Request-Line
                let requestLine = headers[0].split(' ');
                ...
                const request = new HttpRequest(headerMap, pSocket);
        ...
        onRequest_: function (request) {
            const type = request.headers['Upgrade'] ? 'upgrade' : 'request';
            const keepAlive = request.headers['Connection'] == 'keep-alive';
    ...
    HttpRequest.prototype = {
        ...
        write: function (data) {
            if (this.responseHeaders_['Transfer-Encoding'] == 'chunked') {
                ...
                const chunkLength = byteLength.toString(16).toUpperCase() + newline;
                ...
                for (var i = 0; i < newline.length; i++) {
                    bufferView[chunkLength.length + byteLength + i] = newline.charCodeAt(
                        i,
                    );
                }
                data = buffer;
        ...
        serveUrl: function (url) {
            ...
            xhr.onloadend = function () {
                ...
                if (this.getResponseHeader('Content-Type')) {
                    ...
                    const extension = url.substr(url.indexOf('.') + 1);
    ...
    WebSocketRequest.prototype = {
        ...
        accept: function () {
            ...
            let clientKey = this.headers['Sec-WebSocket-Key'];
            ...
            const responseKey = btoa(toString(sha1.digest()));
            ...
            if (this.headers['Sec-WebSocket-Protocol']) {
                responseHeader['Sec-WebSocket-Protocol'] = this.headers[
                    'Sec-WebSocket-Protocol'
                    ];
            }
            this.writeHead(101, responseHeader);
            const socket = new WebSocketServerSocket(this.pSocket_);
    ...
    WebSocketServerSocket.prototype = {
        ...
        sendFrame_: function (op, data) {
            const t = this;
            const WebsocketFrameData = function (op, data) {
                let ary = data;
                if (typeof data == 'string' || data instanceof String) {
                    ary = utf82ary(data);
                }
                ...
                const dataStart = lengthBytes + 2;
                for (var i = 0; i < ary.length; i++) {
                    bv[dataStart + i] = ary[i];
                }
                return buffer;
            };
            const array = WebsocketFrameData(op, data || '');
            this.pSocket_
                .write(array)
                .then(function (bytesWritten) {
